		...
		<article>
					...
					<tr>
						...
						<td class="left">
							<c:if test="${dto.re_lev > 0 }">
								<c:forEach var="i" begin="0" end="${dto.re_lev - 1}" step="1">
									<img alt="1" src="./images/board/level.gif">
								</c:forEach>
								<img alt="2" src="./images/board/re.gif">
							</c:if>
							<c:if test="${!empty dto.file }">
								<img alt="1" src="./images/board/save.png" width="10">
							</c:if>
							<a href="./BoardContent.bo?bno=${dto.bno }&pageNum=${pageNum}&search=${param.search}">${dto.getSubject() }</a>
						</td>
						<td>${dto.getName() }</td>
						<td class="cdate">${dto.getDate() }</td>
			...
			<div id="page_control">
				<c:if test="${startPage != 1}">
					<a href="./BoardList.bo?pageNum=${1 }&search=${param.search}">[First]</a>
				</c:if>
				...
				<%-- <c:if test="${!(endPage >= pageCount)}">
					<a href="./BoardList.bo?pageNum=${(int)(count / pageBlock) + count % pageBlock}">[Last]</a>
				</c:if> --%>
			</div>
		</article>
		<!-- 게시판 -->
